...
contract CamelotFactory is  {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        bytes memory bytecode = type().creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(pair != address(0), ": FAILED");
    ...
    function setFeePercentOwner(address _feePercentOwner) external onlyOwner {
        require(_feePercentOwner != address(0), ": zero address");
    ...
    function setSetStableOwner(address _setStableOwner) external {
        require(msg.sender == setStableOwner, ": not setStableOwner");
        require(_setStableOwner != address(0), ": zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(newOwnerFeeShare > 0, ": ownerFeeShare mustn't exceed minimum");
        require(newOwnerFeeShare <= OWNER_FEE_SHARE_MAX, ": ownerFeeShare mustn't exceed maximum");
    ...
    function setReferrerFeeShare(address referrer, uint referrerFeeShare) external onlyOwner {
        ...
        require(referrerFeeShare <= REFERER_FEE_SHARE_MAX, ": referrerFeeShare mustn't exceed maximum");
