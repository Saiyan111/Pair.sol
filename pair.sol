...
contract  is , UniswapV2ERC20 {
  ...
  */
  function setFeePercent(uint16 newToken0FeePercent, uint16 newToken1FeePercent) external lock {
    require(msg.sender == (factory).feePercentOwner(), ": only factory's feeAmountOwner");
    require(newToken0FeePercent <= MAX_FEE_PERCENT && newToken1FeePercent <= MAX_FEE_PERCENT, ": feePercent mustn't exceed the maximum");
    require(newToken0FeePercent > 0 && newToken1FeePercent > 0, ": feePercent mustn't exceed the minimum");
  ...
  function setStableSwap(bool stable, uint112 expectedReserve0, uint112 expectedReserve1) external lock {
    require(msg.sender == ICamelotFactory(factory).setStableOwner(), ": only factory's setStableOwner");
    require(!pairTypeImmutable, ": immutable");

    require(stable != stableSwap, ": no update");
    require(expectedReserve0 == reserve0 && expectedReserve1 == reserve1, ": failed");
  ...
  function setPairTypeImmutable() external lock {
    require(msg.sender == (factory).owner(), ": only factory's owner");
    require(!pairTypeImmutable, ": already immutable");
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
    ...
    (uint ownerFeeShare, address feeTo) = (factory).feeInfo();
  ...
  function _swap(TokensData memory tokensData, address to, bytes memory data, address referrer) internal lock {
    require(tokensData.amount0Out > 0 || tokensData.amount1Out > 0, ': INSUFFICIENT_OUTPUT_AMOUNT');

    (uint112 _reserve0, uint112 _reserve1, uint16 _token0FeePercent, uint16 _token1FeePercent) = getReserves();
    require(tokensData.amount0Out < _reserve0 && tokensData.amount1Out < _reserve1, ': INSUFFICIENT_LIQUIDITY');


    {
      require(to != tokensData.token0 && to != tokensData.token1, ': INVALID_TO');
    ...
    {// scope for referer/stable fees management
      ...
      uint referrerInputFeeShare = referrer != address(0) ? Factory(factory).referrersFeeShare(referrer) : 0;
      ...
      if(stableSwap){
        (uint ownerFeeShare, address feeTo) = Factory(factory).feeInfo();
        if(feeTo != address(0)) {
  ...
  function sync() external lock {
    ...
    require(token0Balance != 0 && token1Balance != 0, ": liquidity ratio not initialized");
  ...
  */
  function drainWrongToken(address token, address to) external lock {
    require(msg.sender == ICamelotFactory(factory).owner(), ": only factory's owner");
